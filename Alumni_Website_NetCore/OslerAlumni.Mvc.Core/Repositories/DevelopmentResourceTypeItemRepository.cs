...
{
    public class DevelopmentResourceTypeItemRepository
        : IDevelopmentResourceTypeItemRepository
    {
        ...
        public DevelopmentResourceTypeItemRepository(
        ...
        public List<CustomTable_DevelopmentResourceTypeItem> GetAllDevelopmentResourceTypeItems(
            string cultureName = null)
        {
            ...
            {
                CacheKey = GlobalConstants.Caching.DevelopmentResources.DevelopmentResourceTypeItemsAll,
                CultureCode = cultureName,
                IsCultureSpecific = true,
                IsSiteSpecific = false,
                // Bust the cache whenever any of the Development Resource Type custom table items is modified
                CacheDependencies = new List<string>
                {
                    $"customtableitem.{CustomTable_DevelopmentResourceTypeItem.CLASS_NAME}|all"
                }
            };
            ...
            var result = _cacheService.Get(
                cp =>
                {
                    var resourceTypeItems = _customTableRepository
                        .GetAllCustomTableItems<CustomTable_DevelopmentResourceTypeItem>(
                            GetColumns(),
                            orderBy: nameof(CustomTable_DevelopmentResourceTypeItem.ItemOrder));
                    ...
                    var resStringKeys = resourceTypeItems
                        .GetResourceStringCacheKeys(
                            nameof(CustomTable_DevelopmentResourceTypeItem.DisplayName));
        ...
        public CustomTable_DevelopmentResourceTypeItem GetByCodeName(
            string codeName,
            string cultureName = null)
        {
            return GetAllDevelopmentResourceTypeItems(cultureName)?
                .FirstOrDefault(rti =>
                    string.Equals(rti.CodeName, codeName, StringComparison.OrdinalIgnoreCase));
        }

        public List<CustomTable_DevelopmentResourceTypeItem> GetByCodeNames(IEnumerable<string> codeNames, string cultureName = null)
        {
