...
namespace OslerAlumni.Mvc.Core.Repositories
{
    public class CookieStoreRepository : ICookieStoreRepository
    {
        ...
        public CookieStoreRepository(IHttpContextAccessor httpContextAccessor)
        {
        ...
        public T Get<T>() where T : new()
        {
            ...
            var cookie = _httpContextAccessor.HttpContext.Request.Cookies[GetCookieName(obj)];

            if (cookie == null || string.IsNullOrEmpty(cookie) || cookie.Count() <= 0)
            {
                return obj;
            }

            foreach (var key in cookie.Cast<string>().Where(key => objType.GetProperty(key) != null))
            {
                var prop = objType.GetProperty(key);
                object value = cookie[key];
