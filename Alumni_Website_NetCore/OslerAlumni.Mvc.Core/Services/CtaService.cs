...
namespace OslerAlumni.Mvc.Core.Services
{
    public class CtaService : ServiceBase, ICtaService
    {
        ...
        public CtaService(
            UserManager<ApplicationUser> userManag,
        ...
        public IEnumerable<CtaViewModel> GetTopWidgetZoneCtasAsync(TreeNode page)
        {
            return GetCtas(
                page,
            ...
            string fieldName, bool replaceWithDefaultCtas = false)
        {
            ...
            var cacheParameters = new CacheParameters
            {
                CacheKey = string.Format(
                    ...
                    page.NodeGUID,
                    fieldName,
                    _userManager.IsInRoleAsync(new ApplicationUser((UserInfo)_userRepository.CurrentUser), GlobalConstants.Roles.CompetitorRole)),
            ...
            linkedPages = linkedPages?.Where(linkedPage =>
            {
                if (linkedPage is ICompetitorProtected)
                {
                    ...
                    if (competitorProtectedPage.HideFromCompetitors &&
                       _userManager.IsInRoleAsync(new ApplicationUser((UserInfo)_userRepository.CurrentUser), GlobalConstants.Roles.CompetitorRole).GetAwaiter().GetResult())
                    {
                        return false;
                    }
                }
